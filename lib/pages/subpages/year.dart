...
class YearReport extends StatelessWidget {
  ...
  (dio.httpClientAdapter as DefaultHttpClientAdapter).onHttpClientCreate =
    ...
    return client;
        ...
        String url =
            'https://${base}/wp-json/wc/v3/reports/sales?period=year&consumer_key=${ck}&consumer_secret=${cs}';
        Response response = await dio.get(url);
        orders = response.data;
        print(orders![0]['total_sales']);
        for (var i in orders![0]['totals'].keys) {
          dts1.add(i);
          dts.add(DateTime.utc(
              2023,
              int.parse(i.toString().substring(
                    5,
                  ))).toString().split(' ')[0]);
        }
